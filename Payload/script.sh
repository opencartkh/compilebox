...
#Branch 1
if [ "$output" = "" ]; then
    time -v -o /usercode/timing $compiler /usercode/$file -< $"/usercode/inputFile" #| tee /usercode/output.txt
#Branch 2
else
	...
	#Branch 2a
	if [ $? -eq 0 ];then
	    time -v -o /usercode/timing $output -< $"/usercode/inputFile" #| tee /usercode/output.txt    
	    #Branch 2b
	    else
